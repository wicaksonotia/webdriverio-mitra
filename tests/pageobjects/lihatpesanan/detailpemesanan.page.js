...
class DetailPemesananPage extends Page {
    ...
    get tabMenu () { return $('#mat-tab-label-1-1') }

    get btnUbah () { return $('span.pl-8') }
    get jumlahProduk () { return $('td:nth-child(7) > div > span') }
    get editJumlah () { return $('input[formcontrolname="amount"]') }
    get btnSimpan () { return $('tr:nth-child(1) > td:nth-child(9) > div') }
    get btnSubmit () { return $('button.mat-raised-button') }

    //status
    get pesananDilihat () { return $('[class="status text-truncate h6 p-4 status-order mat-pesanan-dilihat-color"]') }
    get diproses () { return $('[class="status text-truncate h6 p-4 status-order mat-diproses-color"]') }
    get konfirmasiPerubahan () { return $('[class="status text-truncate h6 p-4 status-order mat-konfirmasi-perubahan-color"]') }
    get perubahanDisetujui () { return $('[class="status text-truncate h6 p-4 status-order mat-perubahan-disetujui-color"]') }
    get siapDiambil () { return $('[class="status text-truncate h6 p-4 status-order mat-siap-diambil-color"]') }
    get dalamPengiriman () { return $('[class="status text-truncate h6 p-4 status-order mat-dalam-pengiriman-color"]') }
    get pesananDiterima () { return $('[class="status text-truncate h6 p-4 status-order mat-pesanan-diterima-color"]') }
    get selesai () { return $('[class="status text-truncate h6 p-4 status-order mat-selesai-color"]') }
    
    get drpdwnStatusPemesanan () { return $('div.mat-select-trigger') }
    get selectStatusPemesanan () { return $$('mat-option[role="option"] > span') }
    get btnPerbaruiStatus () { return $('button.mat-accent') }
    ...
    async showQuantityBefore () {
        await console.log('Jumlah produk sebelum direvisi: '+await this.jumlahProduk.getText())
    }
