...
allureReporter.addTestId('[MITRA] 19. Create Program STW Mitra (Audience - Pilih Semua)');
allureReporter.addFeature('Create Program STW Mitra (Audience - Pilih Semua)');
...
When (/^I click button Simpan in tab Detail Program form$/, async () => {
    await mitraSuperPuterCreatePage.SimpanDetailProgram()
});  

Then (/^System save data in tab Detail Program form$/, async () => {
    await mitraSuperPuterCreatePage.DetailProgramPage()
});
...
When (/^I select radio button Pilih Semua$/, async () => {
    await mitraSuperPuterCreatePage.pilihSemua()
});

Then (/^System should be able to select radio button Pilih Semua$/, async () => {
    await mitraSuperPuterCreatePage.AudienceTabPage()
});

When (/^I click button Update Semua$/, async () => {
    await mitraSuperPuterCreatePage.updateAudience()
});

Then (/^System should be able to Update Audience$/, async () => {
    await mitraSuperPuterCreatePage.AudienceTabPage()
});
