...
allureReporter.addTestId(
  "[MITRA] 20. Create Program STW Mitra (Audience - Berdasarkan Tier)"
);
allureReporter.addFeature(
  "Create Program STW Mitra (Audience - Berdasarkan Tier)"
);
...
When(
  ...
  async (minimum_nilai_transaksi) => {
    await mitraSuperPuterCreatePage.minimumNilaiTransaksi(
...
When(
  /^I input Limit Spin Keseluruhan (.*)$/,
  async (limit_spin_keseluruhan) => {
    await mitraSuperPuterCreatePage.limitSpinKeseluruhan(
      limit_spin_keseluruhan
    );
  }
);
...
When(
  /^I input Minimum Frekuensi Belanja B2B Mingguan (.*)$/,
  async (minimum_frekuensi_belanja) => {
    await mitraSuperPuterCreatePage.minimumFrekuensiBelanja(
      minimum_frekuensi_belanja
    );
  }
);

Then(
  /^System should be able to input Minimum Frekuensi Belanja B2B Mingguan$/,
  async () => {
    await mitraSuperPuterCreatePage.DetailProgramPage();
  }
);
...
When(
  /^I input Maksimal Frekuensi Reward Per Minggu (.*)$/,
  async (maksimal_frekuensi_reward) => {
    await mitraSuperPuterCreatePage.maksimalFrekuensiReward(
      maksimal_frekuensi_reward
    );
  }
);

Then(
  /^System should be able to input Maksimal Frekuensi Reward Per Minggu$/,
  async () => {
    await mitraSuperPuterCreatePage.DetailProgramPage();
  }
);
...
Then(
  /^System should be able click tier and click button update tier$/,
  async () => {
    await mitraSuperPuterCreatePage.AudienceTabPage();
  }
);
...
Then(
  /^system should be able to click button publish spin the wheel$/,
  async () => {
    await mitraSuperPuterCreatePage.pengaturanMekanismePage();
  }
);
