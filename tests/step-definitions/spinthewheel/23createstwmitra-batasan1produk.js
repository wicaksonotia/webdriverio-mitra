...
allureReporter.addTestId('[MITRA] 23. Create Program STW Mitra - Batasan 1 Produk');
allureReporter.addFeature('Create Program STW Mitra - Batasan 1 Produk');
...
Then (/^system should be able to input min Value$/, async () => {
    await mitraSuperPuterCreatePage.DetailProgramPage()
});
...
Then (/^System should be able to select checkbox tier$/, async () => {
    await mitraSuperPuterCreatePage.AudienceTabPage()
});
...
Then (/^System should be able to select tier gold$/, async () => {
    await mitraSuperPuterCreatePage.AudienceTabPage()
});
